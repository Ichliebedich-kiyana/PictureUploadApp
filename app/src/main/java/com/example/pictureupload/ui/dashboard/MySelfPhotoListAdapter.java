...
public class MySelfPhotoListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int TYPE_CONTENT = 0;
    private static final int TYPE_FOOTER = 1;

    private List<MySelfBean.DataBean.RecordsBean> dataList;

    private ProgressBar pbLoading;
    private TextView tvLoadMore;

    public MySelfPhotoListAdapter() {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (type == TYPE_CONTENT) {
            MySelfBean.DataBean.RecordsBean recordsBean = dataList.get(position);
            Glide.with(context)
            ...
            if (recordsBean.getImageUrlList().isEmpty()) {
                ...
                for (int i = 0; i < recordsBean.getImageUrlList().size(); i++) {
                    Glide.with(context)
...
//                }
     ...
     */
    public void addAll(List<MySelfBean.DataBean.RecordsBean> list) {
     ...
     * 将新增数据集添加到前面，一般用作刷新
     */
    public void addFirstAll(List<MySelfBean.DataBean.RecordsBean> list) {
