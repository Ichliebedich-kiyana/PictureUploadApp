...
public class PhotoDetailActivity2 extends AppCompatActivity {
    ...
    private void initData() {
        ...
        collectIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                isCollect = !isCollect;
                if (isCollect) {    // 收藏
                    HttpUtil.getInstance().doPost("/collect?shareId=" + recordsBean.getId() + "&userId=" + MyApplication.getSp().getString("id", ""), null, new OkCallBack() {
                        @Override
        ...
        likeIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                isLike = !isLike;
                if (isLike) {    // 点赞
                    HttpUtil.getInstance().doPost("/like?shareId=" + recordsBean.getId() + "&userId=" + MyApplication.getSp().getString("id", ""), null, new OkCallBack() {
                        @Override
                        public void onSuccess(String response) throws IOException {
                            runOnUiThread(new Runnable() {
                                @Override
                ...
                HttpUtil.getInstance().doPost("/share/save", bodyMap, new OkCallBack() {
                    ...
                    public void onSuccess(String response) throws IOException {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(PhotoDetailActivity2.this, "保存成功", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                    ...
                    public void onFail(String response) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(PhotoDetailActivity2.this, "保存失败", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                });
    ...
    private void showDetail() {
        HttpUtil.getInstance().doGet("/share/detail?shareId=" + recordsBean.getId() + "&userId=" + MyApplication.getSp().getString("id", ""), new OkCallBack() {
            @Override
            public void onSuccess(String response) throws IOException {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        isLike = data.isHasLike();
                        isCollect = data.isHasCollect();
                        if (isLike) {
